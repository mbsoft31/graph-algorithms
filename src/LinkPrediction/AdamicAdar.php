 ...
 *
 * The Adamic-Adar index is a measure of the similarity between two nodes
 * based on the common neighbors they share, weighted by the rarity of those neighbors.
 *
 * For nodes u and v, the index is:
 * AA(u,v) = sum over common neighbors w of 1/log(deg(w))
 *
 * Higher scores indicate stronger likelihood of a link forming.
 */
final class AdamicAdar implements LinkPredictorInterface
{
    /**
     * Compute the Adamic-Adar score between two nodes.
     */
    public function score(GraphInterface $graph, string $u, string $v): float
    {
        // Need predecessors for directed graphs to get undirected view
        ...
        foreach ($commonNeighbors as $wIdx) {
            ...
            if ($wDegree > 1) { // Avoid division by zero and log(1) = 0
                $score += 1.0 / log($wDegree);
            }
        }
    ...
    /**
     * Compute top-k Adamic-Adar scores from a given node.
     */
